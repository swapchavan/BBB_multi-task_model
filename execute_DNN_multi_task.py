...
def main():
    ...
    mc_set_values = pd.read_csv(args.training_dataset_tox_values_path, header = None, index_col = None)
    mc_set_values.columns = ['VALUES']
    ## logBB value conversion
    # if logBB is log base 2, make exponential of 2
    mc_set_values['VALUES'] = 2 ** mc_set_values['VALUES']
    ...
    pre_train_index, test_index = train_test_split(mc_set_classes.index, test_size=0.1, stratify=mc_set_classes.CLASSES, random_state=1234)
    # slice pre-train set 
    ...
    #pd.DataFrame(X_pre_train.index).to_csv('X_TRAIN_VAL.csv', header=['SMILES'], index=False, sep=',')
    #Y_class_pre_train.to_csv('Y_class_TRAIN_VAL.csv', header=True, index=False, sep=',')
    #Y_value_pre_train.to_csv('Y_value_TRAIN_VAL.csv', header=True, index=False, sep=',')
    ...
    Y_class_test.to_csv('Y_class_TEST.csv', header=True, index=False, sep=',')
    Y_value_test.to_csv('Y_value_TEST.csv', header=True, index=False, sep=',')
    ...
    rskf = RepeatedStratifiedKFold(n_splits=n_fold_CV, n_repeats=number_of_repeats, random_state=12345)
    i = 0
    # run 5 fold CV
    for train_indices, val_indices in rskf.split(X_pre_train, Y_class_pre_train):
        ...
        #VAL_dataset = batch_dataset(X_VAL, Y_CLASS_VAL, Y_VALUE_VAL)
        ...
        #TEST_dataset = TensorDataset(torch.Tensor(X_test.values).float(), torch.Tensor(Y_class_test.values).float(), torch.Tensor(Y_value_test.values).float())
        ...
        #test_loader = DataLoader(TEST_dataset, batch_size=BATCH_SIZE, shuffle=True, pin_memory=True, drop_last=True, num_workers=args.num_workers)
        ...
        optimizer = optim.AdamW(my_model.parameters(), lr=0.0005, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.000001, amsgrad=False)
        ...
        temp_seed = i
        train_ROC, train_R2, val_ROC, val_R2, test_ROC, test_R2 = train_model(n_EPO, my_model, train_loader, val_loader, X_test, Y_class_test, Y_value_test, loss_func_regress, loss_func_classify, optimizer, scheduler, device, CV_dir_subfolder_2, temp_seed, loss_adj_factor)
